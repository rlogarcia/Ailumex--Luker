...
class PortalCoachController(CustomerPortal):
    ...
    def portal_coach_agenda(self, start=None, **kwargs):
        """
        Vista de horario semanal del coach
    ...
    def mark_attendance(self, session_id, enrollment_id, status, **kwargs):
        ...
        """
        request_id = f"attendance-{session_id}-{enrollment_id}"
        _logger.info(f"[{request_id}] Marcando asistencia: status={status}, user={request.env.user.login}")
        
        try:
            # Verificar que el coach es dueño de la sesión
            coach = self._get_coach()
            if not coach:
                _logger.warning(f"[{request_id}] Usuario sin perfil de coach")
                return {'success': False, 'error': 'No autorizado - Usuario sin perfil de coach'}

            teacher = self._get_coach_employee(coach)
            if not teacher:
                _logger.warning(f"[{request_id}] Coach sin employee asociado")
                return {'success': False, 'error': 'No autorizado - Coach sin employee'}
            ...
            if session.state not in ['started', 'done']:
                _logger.warning(f"[{request_id}] Sesión en estado incorrecto: {session.state}")
                return {'success': False, 'error': f'Solo se puede marcar asistencia en sesiones iniciadas (estado actual: {session.state})'}
            ...
            if not enrollment:
                _logger.warning(f"[{request_id}] Inscripción no encontrada")
                return {'success': False, 'error': 'Inscripción no encontrada'}

            # Validar estado
            if status not in ['attended', 'absent']:
                _logger.warning(f"[{request_id}] Estado inválido: {status}")
                return {'success': False, 'error': f'Estado inválido: {status}'}
            
            old_state = enrollment.state
            
            # Marcar asistencia usando write() para disparar hooks de sincronización
            enrollment.write({'state': status})
            
            _logger.info(f"[{request_id}] Asistencia actualizada: {old_state} -> {status} para estudiante {enrollment.student_id.name}")
            ...
            _logger.error(f"[{request_id}] Error al marcar asistencia: {str(e)}", exc_info=True)
    ...
    def upload_session_files(self, session_id, **post):
        ...
        """
        request_id = f"upload-{session_id}-{request.env.user.id}"
        _logger.info(f"[{request_id}] Iniciando upload de archivos para sesión {session_id}")
        
        try:
            coach = self._get_coach()
            if not coach:
                _logger.warning(f"[{request_id}] Usuario no autorizado: {request.env.user.login}")
            ...
            teacher = self._get_coach_employee(coach)
            if not teacher:
                _logger.warning(f"[{request_id}] Coach sin employee asociado: {coach.name}")
                return request.make_response(
                    json.dumps({'success': False, 'error': 'No autorizado'}),
                    headers=[('Content-Type', 'application/json')]
                )
            ...
            for key in request.httprequest.files:
                ...
                if file and file.filename:
                    ...
                    if file_size > max_size:
                        _logger.warning(f"[{request_id}] Archivo rechazado (tamaño {file_size/1024/1024:.2f}MB): {file.filename}")
                        return request.make_response(
                            json.dumps({'success': False, 'error': f'Archivo {file.filename} excede 10MB'}),
                            headers=[('Content-Type', 'application/json')]
                        )
                    ...
                    _logger.info(f"[{request_id}] Archivo subido: '{file.filename}' ({file_size/1024:.2f}KB) - Attachment ID: {attachment.id}")
            
            if file_count == 0:
                _logger.warning(f"[{request_id}] No se recibieron archivos en el request")
                return request.make_response(
                    json.dumps({'success': False, 'error': 'No se recibieron archivos'}),
                    headers=[('Content-Type', 'application/json')]
                )
            
            _logger.info(f"[{request_id}] Upload exitoso: {file_count} archivos, {total_size/1024:.2f}KB total")
            ...
            _logger.error(f"[{request_id}] Error al subir archivos: {str(e)}", exc_info=True)
            return request.make_response(
                json.dumps({'success': False, 'error': f'Error del servidor: {str(e)}'}),
                headers=[('Content-Type', 'application/json')]
            )

    @http.route('/my/coach/session/<int:session_id>/save_session_observations', type='json', auth='user', methods=['POST'], website=True)
    def save_session_observations(self, session_id, general_observations=None, novedad_observations=None, novedad_type=None, **kwargs):
        """
        Guarda las observaciones de una sesión
        
        Args:
            session_id: ID de la sesión
            general_observations: Observaciones generales de la sesión
        ...
        """
        try:
            _logger.info(f"=== GUARDANDO OBSERVACIONES SESIÓN {session_id} ===")
            _logger.info(f"novedad_type: {novedad_type}, novedad_observations: {novedad_observations}")
            
            coach = self._get_coach()
            if not coach:
                _logger.error("No se pudo obtener el coach")
                return {'success': False, 'error': 'No autorizado'}

            teacher = self._get_coach_employee(coach)
            if not teacher:
                _logger.error("No se pudo obtener el empleado docente")
                return {'success': False, 'error': 'No autorizado'}
            ...
            if not session:
                _logger.error(f"Sesión {session_id} no encontrada o no pertenece al docente {teacher.id}")
                return {'success': False, 'error': 'Sesión no encontrada'}

            # Guardar observaciones en campos temporales de la sesión
            update_vals = {}
            # No guardamos general_observations porque ese campo no existe en el modelo
            # Solo guardamos novelty_observations (plural) y novelty_type
            if novedad_observations is not None:
                update_vals['novelty_observations'] = novedad_observations
            if novedad_type is not None:
                update_vals['novelty_type'] = novedad_type
                
            if update_vals:
                _logger.info(f"Escribiendo en sesión: {update_vals}")
                session.write(update_vals)
                _logger.info(f"Valores guardados exitosamente en sesión {session_id}")
            else:
                _logger.warning(f"No hay valores para guardar en sesión {session_id}")
            ...
            _logger.error(f"ERROR al guardar observaciones de sesión {session_id}: {str(e)}", exc_info=True)
            return {'success': False, 'error': f'Error al guardar: {str(e)}'}

    @http.route('/my/coach/session/<int:session_id>/get_session_observations', type='json', auth='user', methods=['POST'], website=True)
    ...
    def finish_session(self, session_id, novedad_type=None, observaciones=None, **kwargs):
        ...
        try:
            ...
            try:
                ...
                attended_enrollments = session.enrollment_ids.filtered(lambda e: e.state == 'attended')
                attended_count = len(attended_enrollments)
                
                _logger.info(f"Sesión {session.id}: {attended_count} estudiantes asistieron de {len(session.enrollment_ids)} inscritos")
                
                # Obtener adjuntos de la sesión para transferir a bitácora
                # Buscar adjuntos vinculados a la sesión académica
                session_attachments = request.env['ir.attachment'].sudo().search([
                    ('res_model', '=', 'benglish.academic.session'),
                    ('res_id', '=', session.id),
                ])
                
                # IMPORTANTE: También buscar adjuntos en el campo Many2many de novelty_attachment_ids
                if hasattr(session, 'novelty_attachment_ids') and session.novelty_attachment_ids:
                    session_attachments |= session.novelty_attachment_ids
                
                _logger.info(f"Sesión {session.id}: {len(session_attachments)} archivos adjuntos para transferir")
                
                # CASO 1: Al menos un estudiante asistió - crear registro por cada uno
                if attended_count > 0:
                    for enrollment in attended_enrollments:
                        ...
                        # Vincular adjuntos al registro de bitácora
                        if session_attachments:
                            history_record.write({'attachment_ids': [(6, 0, session_attachments.ids)]})
                        
                        _logger.info(f"Creado registro de bitácora ID {history_record.id} para estudiante {enrollment.student_id.name} con {len(session_attachments)} adjuntos")
                
                # CASO 2: NINGÚN estudiante asistió pero hay novedad/materiales - crear registro sintético
                elif attended_count == 0 and (novedad_type or len(session_attachments) > 0):
                    _logger.warning(f"Sesión {session.id}: Ningún estudiante asistió pero hay novedad='{novedad_type}' o {len(session_attachments)} adjuntos - creando registro sintético")
                    
                    # Crear un único registro de bitácora SIN estudiante (documentación de sesión)
                    history_vals_synthetic = {
                        # Relaciones principales
                        'student_id': False,  # Sin estudiante específico
                        'session_id': session.id,
                        'enrollment_id': False,
                        ...
                        'session_time_end': session.time_end,
                        'program_id': session.program_id.id,
                        'level_id': False,  # Sin nivel porque no hay estudiante específico
                        'subject_id': session.subject_id.id,
                        'teacher_id': session.teacher_id.id,
                        'campus_id': session.campus_id.id,
                        'group_id': session.group_id.id if session.group_id else False,
                        ...
                        # Estado de asistencia
                        'attendance_status': 'absent',  # Marcado como ausencia general
                        
                        # Novedad y observaciones
                        'novedad': novedad_value,
                        'notes': f"{notes_text}\n\nNOTA: Ningún estudiante asistió a esta sesión. Registro creado para documentación de materiales/novedades.",
                        
                        # Sin calificación
                        'grade': False,
                    }
                    
                    history_record_synthetic = request.env['benglish.academic.history'].sudo().create(history_vals_synthetic)
                    
                    # Vincular adjuntos al registro sintético
                    if session_attachments:
                        history_record_synthetic.write({'attachment_ids': [(6, 0, session_attachments.ids)]})
                    
                    _logger.info(f"Creado registro sintético de bitácora ID {history_record_synthetic.id} para sesión {session.id} sin estudiantes (novedad={novedad_type}, {len(session_attachments)} adjuntos)")
                
                else:
                    _logger.info(f"Sesión {session.id}: Ningún estudiante asistió y no hay novedad/adjuntos - no se crea registro en bitácora")
                
                _logger.info(f"Registros de bitácora procesados exitosamente para sesión {session.id}")
            ...
            # Determinar mensaje de éxito basado en lo que se registró
            attended_count = len(session.enrollment_ids.filtered(lambda e: e.state == 'attended'))
            has_materials = len(session_attachments) > 0 if 'session_attachments' in locals() else False
            
            if attended_count > 0:
                success_message = f'Clase terminada correctamente. Registrados {attended_count} estudiantes en bitácora.'
            elif novedad_type or has_materials:
                success_message = f'Clase terminada. Documentación registrada en bitácora (sin asistencia de estudiantes).'
            else:
                success_message = 'Clase terminada correctamente.'
            
            _logger.info(f"Sesión {session.id} finalizada exitosamente: {success_message}")
