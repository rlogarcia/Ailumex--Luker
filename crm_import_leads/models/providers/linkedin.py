from typing import Dict, Iterable
from odoo.exceptions import UserError
from .base import ProviderBase, ProviderRegistry

@ProviderRegistry.register
class LinkedInProvider(ProviderBase):
    key = 'linkedin'
    label = 'LinkedIn Lead Gen Forms'

    def fetch(self, limit: int = 100) -> Iterable[Dict]:
        # Obtener la librería requests y la fuente
        requests = self.ensure_requests()
        source = self.source
        object_id = source.object_id
        if not object_id:
            # Validar que el ID del formulario esté configurado
            raise UserError('Debes indicar el ID del formulario en LinkedIn.')

        headers = {
            'Authorization': f'Bearer {self.get_token()}',
        }
        params = {
            'q': 'form',
            'form': f'urn:li:leadGenForm:{object_id}',
            'count': limit,
        }
        # Si hay una última sincronización, pedir sólo leads posteriores
        if source.last_sync:
            params['timeIntervals.timeGranularityType'] = 'DAY'
            params['timeIntervals.timeRange.start'] = int(source.last_sync.timestamp() * 1000)

        url = 'https://api.linkedin.com/v2/leadFormResponses'
        response = requests.get(url, headers=headers, params=params, timeout=30)
        response.raise_for_status()
        data = response.json() or {}
        # Devolver la lista de respuestas (clave 'elements' en la respuesta)
        return data.get('elements', [])

    def map_payload(self, payload: Dict[str, Dict]) -> Dict[str, str]:
        # Extraer respuestas del payload
        answers = payload.get('answers', [])
        values: Dict[str, str] = {}
        for answer in answers:
            question = (answer.get('question', {}) or {}).get('entity') or ''
            value = answer.get('answer') or {}
            field_value = value.get('value') if isinstance(value, dict) else value
            # Normalizar nombre de campo a minúsculas para mapear fácilmente
            values[question.lower()] = field_value

        # Construir campos esperados por el wizard de importación
        full_name = values.get('full_name') or ' '.join(filter(None, [values.get('first_name'), values.get('last_name')]))
        company = values.get('company_name') or ''
        phone = values.get('phone_number') or ''
        email = values.get('email') or ''
        country = values.get('country') or ''

        # Si no hay información mínima, ignorar el payload
        if not full_name and not email and not phone:
            return {}

        # Devolver un mapeo con claves en español que usa el wizard
        return {
            'Nombre': full_name or company or email,
            'Correo': email,
            'Telefono': phone,
            'Empresa': company,
            'Pais': country,
        }
