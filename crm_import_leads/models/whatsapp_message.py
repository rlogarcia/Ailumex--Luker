...
class WhatsAppMessage(models.Model):
    ...
    def action_send(self):
        """Send WhatsApp message via OCA mail.gateway"""
        for rec in self:
            if rec.state != "draft":
                continue
            ...
            try:
                ...
                try:
                    phone_obj = phonenumbers.parse(rec.phone, None)
                    normalized_phone = phonenumbers.format_number(
                        phone_obj, phonenumbers.PhoneNumberFormat.E164
                    )
                except:
                    normalized_phone = rec.phone

                # Buscar o crear canal de discuss para este número
                channel = self.env["discuss.channel"].search(
                    [
                        ("gateway_id", "=", rec.gateway_id.id),
                        ("gateway_channel_token", "=", normalized_phone),
                    ],
                    limit=1,
                )
                ...
                if not channel:
                    ...
                    channel = self.env["discuss.channel"].create(
                        {
                            "name": rec.partner_id.name or normalized_phone,
                            "channel_type": "chat",
                            "gateway_id": rec.gateway_id.id,
                            "gateway_channel_token": normalized_phone,
                        }
                ...
                # Enviar mensaje usando el sistema OCA (post en chatter)
...
class WhatsAppTemplate(models.Model):
    ...
    message = fields.Text(
        "Message Template", required=True, help="Use {{field_name}} for variables"
    )
...
class WhatsAppGateway(models.Model):
    ...
    def _send_custom(self, phone, message):
        ...
        data = {"phone": phone, "message": message, "api_secret": self.api_secret}
