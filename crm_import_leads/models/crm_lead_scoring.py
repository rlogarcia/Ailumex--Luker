...
class CrmLead(models.Model):
    ...
    @api.depends('expected_revenue', 'email_from', 'phone', 'name', 'partner_name', 'country_id', 'source_id', 'partner_id')
    def _compute_lead_score(self):
        """Calcula la puntuación del lead aplicando lógica de scoring"""
        for lead in self:
            score = 0
            
            _logger.info(f'Calculando score para lead {lead.name}')
            ...
            if lead.country_id:
                ...
                elif country_code in ['CA', 'GB', 'DE', 'FR']:  # Otros países desarrollados
                    score += 10
                    _logger.info(f'Lead {lead.name}: +10 por país desarrollado ({country_code})')
            ...
            if lead.source_id:
                ...
                if 'Referral' in source_name or 'Referido' in source_name:  # 👥 Referido - Alta calidad
                    score += 15
                    _logger.info(f'Lead {lead.name}: +15 por fuente Referido')
                elif 'Email' in source_name or 'Correo' in source_name:  # 📧 Email Marketing
                    score += 10
    ...
    def get_scoring_details(self):
        ...
        if self.country_id:
            ...
            if country_code == 'US':
                ...
                details.append({'rule': f'País desarrollado ({country_code})', 'points': 10})
    ...
    def create(self, vals_list):
        ...
        for lead in leads:
            if lead.lead_score >= 70 and not lead.high_score_alert_sent:
                lead.with_context(skip_scoring_notifications=True)._check_scoring_notifications()
    ...
    def write(self, vals):
        ...
        score_fields = {'expected_revenue', 'email_from', 'phone', 'name', 'partner_name', 'country_id', 'source_id', 'partner_id'}
        ...
        if not self.env.context.get('skip_scoring_notifications'):
            for lead in self:
                if lead.lead_score >= 70 and not lead.high_score_alert_sent:
                    ...
                    try:
                        lead.with_context(skip_pipeline_automation=True, skip_scoring_notifications=True).sudo().write({'high_score_alert_sent': False})
                    except Exception as e:
                        _logger.warning(f'No se pudo resetear flag high_score_alert_sent: {str(e)}')
    ...
    def _apply_probability_stage_rules(self):
        ...
        stage_probable = self.env.ref('crm_import_leads.crm_stage_probable_close', raise_if_not_found=False)
        if not stage_probable:
            return

        eligible_leads = self.filtered(lambda l: l.type == 'opportunity' and l.active and l.probability >= 80 and not l.stage_id.is_won and not l.stage_id.fold and l.stage_id != stage_probable)
        ...
        for lead in eligible_leads:
            ...
            message = (
                '<p>La oportunidad alcanzó una probabilidad del 80% o más. Se movió automáticamente a <strong>Cierre probable</strong>.</p>'
            )
    ...
    def _reset_inactivity_flag_on_stage_change(self):
        stage_inactive = self.env.ref('crm_import_leads.crm_stage_inactive', raise_if_not_found=False)
        if not stage_inactive:
            return

        to_reset = self.filtered(lambda l: l.inactivity_notification_sent and l.stage_id and l.stage_id.id != stage_inactive.id)
        for lead in to_reset:
            ctx = dict(self.env.context, skip_pipeline_automation=True, skip_inactivity_reset=True)
    ...
    def _handle_auto_close_on_stage_change(self):
        ...
        for lead in self.filtered(lambda l: l.stage_id and l.stage_id == stage_won and not l.auto_close_reason):
    ...
    def _record_auto_close(self, reason):
        ...
        ctx = dict(self.env.context, skip_pipeline_automation=True, skip_auto_close_update=True)
        self.with_context(ctx).sudo().write(values)

    def _get_last_activity_datetime(self):
        self.ensure_one()
        activity_dates = [date for date in self.activity_ids.mapped('write_date') if date]
    ...
    def _set_inactive_stage(self, stage_inactive, inactivity_days):
        for lead in self:
            ...
            if stage_inactive.probability and (lead.probability or 0) > stage_inactive.probability:
    ...
    def _notify_inactivity(self, inactivity_days):
        ...
        last_activity_label = format_datetime(self.env, last_activity) if last_activity else 'sin registros recientes'
        message = (
        ...
        try:
            if partners:
                self.message_notify(partner_ids=partners, body=message, subject='Oportunidad Inactiva')
            else:
            ...
            _logger.exception('No se pudo notificar la inactividad por message_notify/message_post')
    ...
    def _send_high_score_alert(self, email_to='edcamilo2016@gmail.com'):
        ...
        if not self.high_score_alert_sent:
            try:
                ...
                _logger.info(f'✅ Correo de lead prioritario enviado a {email_to} para lead {self.name} (ID: {self.id})')
                ...
                try:
                    ...
                    _logger.warning(f'No se pudo actualizar flag high_score_alert_sent: {str(write_error)}')
                    
            except Exception as e:
                _logger.error(f'❌ Error enviando correo de lead prioritario para {self.name}: {str(e)}')
        else:
            _logger.info(f'ℹ️ Alerta ya enviada previamente para lead {self.name} (ID: {self.id})')
    ...
    def _check_scoring_notifications(self):
        ...
        _logger.info(f'� Verificando notificaciones para lead {self.name} con score {self.lead_score}')
        ...
        if self.user_id:
            ...
            try:
                if self.user_id and self.user_id.partner_id:
                    self.message_notify(partner_ids=[self.user_id.partner_id.id], body=message, subject='Lead de alta prioridad')
                else:
                ...
                _logger.exception('No se pudo enviar notificación de lead prioritario via message_notify')
    ...
    def cron_check_inactive_opportunities(self):
        ...
        param_value = self.env['ir.config_parameter'].sudo().get_param('crm_import_leads.inactivity_days', '10')
        try:
        ...
        for rule in rules:
            if rule.evaluate_condition(self):
                applied_rules.append({
                    ...
                    'field': dict(rule._fields['condition_field'].selection).get(rule.condition_field),
                    'operator': dict(rule._fields['condition_operator'].selection).get(rule.condition_operator),
